...
def ground(c:dict[str,Any])->dict[str,Any]:
    ...
    profiles=[
      (('chocolate','wonka','tasting','confection'),'Food / Confectionery','chocolate tasting and review','premium chocolate assortment, broken fillings, texture detail, and an authentic tasting reaction'),
    ...
    return {'core_subject':subject,'category':category,'content_intent':intent,'key_message':clean(c.get('why_it_matters') or intent,42),'visualizable_information':visual,'narratable_information':intent,'exact_information':[str(c[k]).strip() for k in ('price','date','statistics','rank','quote') if c.get(k)],'brand_information':clean(c.get('brand') or c.get('company'),12),'interaction_information':clean(c.get('cta') or 'Invite one relevant viewer response',18),'emotional_direction':'warm curiosity' if category=='Food / Confectionery' else 'confident analysis' if category in {'Sports','Finance / Business','Gaming'} else 'polished optimism','source_topic':topic}

def strategy(g:dict[str,Any],requested:str)->tuple[str,str,str,str]:
    cat=g['category']
    if cat in {'Food / Confectionery','Beauty','Automotive','Travel'}: out=('REVIEW' if 'review' in g['core_subject'] else 'PRODUCT_STORY','VISUAL_FIRST','PRESENTER_OPTIONAL','PREMIUM')
